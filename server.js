...
- COST-ATTRIBUTION RULE (important for honest ROIs): only count a grid/line-extension cost against electric heat trace when freeze protection is THE reason power would come to the site (e.g. a gas well with no pumpjack/ESP). If the site needs power anyway, treat the extension as sunk — the comparison then runs on installed cost (KK units are typically cheaper than the hazloc electrical scope), operating cost, and reliability. KK's strongest sites: gas wells and satellite wellheads with no other electrical load, pads where the trace runs far from the powered panel (teck cable cost), and sites where added trace load forces transformer/panel upgrades. In Alberta, oil & gas customers (Fortis Rate 45 class) do pay customer contributions for line extensions — but never claim that cost unless it is genuinely attributable.
